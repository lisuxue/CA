...
Dfg::Dfg(Basic_block *bb){
   ...
   while(itmp!=nullptr){
	...
	del=get_delay((*itd)->type, itmp, (*itd)->inst);
	 ...
	 //si on ne l'a pas déja rencontré (si dependance avec des instructions du bloc, donc si ce n'est pas un nop) on lui créé un noeud
	 if (nodeTab[icurr] == nullptr)
	   nodeTab[icurr]=new Node_dfg(itmp);
	 else { 
	   if (icurr > 1) {  
	 ...
	 //on l'ajoute à la liste  des delayed_slots
...
// ajouter à la classe pour  que l'on puisse verifier qu'un saut n'est pas pret avant les autres instructions 
...
// si branchement : chercher les dependances de control avec prédécesseur => instruction potentielle pour le delayed slot 
